...
spark=SparkSession.builder.appName("balance-risk").getOrCreate()
...
redisrawinput =spark.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092") \
    .option("subscribe","redis-server")                  \
    .option("startingOffsets","earliest")\
    .load() 
# TO-DO: cast the value column in the streaming dataframe as a STRING 
redisinput=redisrawinput.selectExpr("cast(key as string) key","cast(value as string) value")

redisinput.withColumn("value",from_json("value",redisserverSchema)).select(col('value.*')).createOrReplaceTempView("RedisSortedSet")
...
zSetDecodedEntriesStreamingDF.withColumn("encodedCustomer",from_json("encodedCustomer",customerSchema)).select(col('encodedCustomer.*')).createOrReplaceTempView("CustomerRecords")
...
stedirawinput =spark.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092") \
    .option("subscribe","stedi-events")                  \
    .option("startingOffsets","earliest")\
    .load() 
                                   
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediinput=stedirawinput.selectExpr("cast(key as string) key","cast(value as string) value")
...
stediinput.withColumn("value", from_json("value",stedieventSchema)).select(col('value.*')).createOrReplaceTempView("CustomerRisk")
...
RiskStreamingDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr(""" email = customer """))
...
# +--------------------+-----+--------------------+---------+
 #InthisJSONFormat{"customer":"Santosh.Fibonnaci@test.com","score":"28.5","email":"Santosh.Fibonnaci@test.com","birthYear":"1963"} 
RiskStreamingDF.selectExpr("cast(email as string) key", "to_json(struct(*)) as value") \
    .writeStream \
    .format("kafka") \
